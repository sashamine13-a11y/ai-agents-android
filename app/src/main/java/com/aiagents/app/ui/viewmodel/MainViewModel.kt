...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun sendMessage(text: String, useMultiAgent: Boolean = false) {
        ...
        if (!apiKeys.value.isAnySet()) {
            _error.value = "Добавьте API ключи в настройках"
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _messages.value += ChatMessage("error", "❌ Ошибка: ${e.message}")
            } finally {
        ...
        try {
            when (tool.toolName) {
                ...
                "LIST_DIR" -> {
                    ...
                    _messages.value += ChatMessage("system", "🔧 Команда: ${tool.toolName}")
                }
            }
        } catch (e: Exception) {
    ...
    fun attachFile(uri: Uri) {
        try {
            ...
            _error.value = "Не удалось прочитать файл: ${e.message}"
        }
    }
}
